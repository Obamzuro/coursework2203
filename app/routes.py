...
from .models import SubjectForm, LabForm, LabResultForm, StudentForm, SkillForm, StudentSkillForm, LoginForm
from . import session
...
def table_view(fn):
    ...
    def view(*args, **kwargs):
        ...
        return render_template('table_instances.html', table_name=table_name,
                               table_instances=table_instances, table_fields=table_fields, user=current_user.is_authenticated)
    return view
...
def add_table_instance_decorator(fn):
    ...
    def view(*args, **kwargs):
        ...
        return render_template('table_instance.html', action="Add",
                               ...
                               table_name=table_name, user=current_user.is_authenticated)
    return view
...
def home_page():
    ...
    return render_template('index.html', plot1=graphsPie, plot2=graphsBar, user=current_user.is_authenticated)
...
def login():
    ...
    return render_template('login.html', title='Sign In', form=form, user=current_user.is_authenticated)
...
def register():
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('home_page'))
    return render_template('register.html', title='Register', form=form, user=current_user.is_authenticated)